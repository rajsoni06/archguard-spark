...
// The original file was appended at the end. We can find the start of the appended original file
// by looking for the last occurrence of the first line of the file (or the imports).
const originalStart = content.lastIndexOf('import type { LucideIcon }');

if (originalStart !== -1 && originalStart > 0) {
    console.log('Found duplicated original file. Restoring...');
    content = content.substring(originalStart);
}

// Now `content` is the original file. Let's do the AWS replacement correctly.
const awsStart = content.indexOf('aws: {');
const categoriesStart = content.indexOf('categories: [', awsStart);
const azureStart = content.indexOf('azure: {');

// The categories array ends right before azure: {
// We want everything up to `categories: [` + new categories + `    ],\n  },\n  ` + everything from `azure: {`
const newCategories = `categories: [
      ...
      },
      {
        ...
        services: [
          ...
          s("waf", "AWS WAF", "CDN & Edge", Shield, ["waf"]),
          s("shield", "AWS Shield", "CDN & Edge", Shield, ["waf"]),
        ],
      },
      {
        name: "Security",
...
const finalContent = content.substring(0, categoriesStart) + newCategories + '\n    ],\n  },\n  ' + content.substring(azureStart);
